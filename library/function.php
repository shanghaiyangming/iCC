 ...
 * @return url
 */
function dealUploadFile($fileName, $filePath, $outId = 0, $meta = array())
{
    $config = Zend_Registry::get('config');
    try {
        $model = new Admin_Model_File();
        $gfs = $model->db->getGridFS();
        
        $fileByte = file_get_contents($filePath);
        $finfo = new finfo(FILEINFO_MIME);
        $fileMime = $finfo->buffer($fileByte);
        $fileInfo = array(
            'fileName' => $fileName,
            'fileMime' => $fileMime,
            'downloads' => (int) 0,
        ...
        $fileId = $gfs->storeBytes($fileByte, $fileInfo);
        
        if ($outId == 0) {
            return $config['global']['server'] . 'soa/file/get/id/' . $fileId;
        }
        return $fileId;
    } catch (Exception $e) {
        logError(exceptionMsg($e));
...
/**
 * 通过URL或者id来删除gridFs上的文件
 *
 * @param string $url            
 * @return bool 成功返回true 失败返回false
 */
function removeFromGridFS($url)
{
    try {
        if (filter_var($url, FILTER_VALIDATE_URL) !== false) {
            ...
            if (strlen($url) == 24) {
                ...
                throw new Exception('Invalid URL or Invalid ID');
                return false;
            }
        
        $model = new Admin_Model_File();
        $gfs = $model->db->getGridFS();
        return $gfs->remove(array(
            '_id' => MongoId($id)
        ));
    } catch (Exception $e) {
        logError(exceptionMsg($e));
